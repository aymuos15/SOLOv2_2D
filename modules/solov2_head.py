...
from .nms import multi_apply, matrix_nms, points_nms
from .losses import dice_loss, py_sigmoid_focal_loss
...
class SOLOv2Head(nn.Module):
    ...
    def forward_single(self, x, idx, eval=False, upsampled_size=None):
        ...
        # Add coordinate features to input
        ins_kernel_feat = torch.cat([x, coord_feat], 1)
        
        # Process kernel branch
        kernel_feat = F.interpolate(ins_kernel_feat, size=self.seg_num_grids[idx], mode='bilinear', align_corners=False)
        cate_feat = kernel_feat[:, :-2, :, :]  # Remove coordinate channels for category branch
    ...
    def solov2_target_single(self,
        ...
        for (lower_bound, upper_bound), stride, num_grid \
            ...
            # For each scale range, it filters the indices of the bounding boxes 
            # whose areas fall within the current range.
            # These indices are stored in hit_indices.
            ...
            for gt_label, half_h, half_w in zip(gt_labels, half_hs, half_ws):
                ...
                # Calculate top, down, left, right box values directly
                ...
                # Replace imrescale with a simple PyTorch interpolation
                if not isinstance(seg_mask, torch.Tensor):
                    seg_mask = torch.tensor(seg_mask, device=device)
                # Add batch and channel dimensions required by F.interpolate
                seg_mask = seg_mask.float().unsqueeze(0).unsqueeze(0)
                # Downsample the mask by a factor of 1/output_stride
